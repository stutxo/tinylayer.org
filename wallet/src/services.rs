...
use bitcoin::{Amount, BlockHash, ScriptBuf, TxOut};
use bitcoincore_rpc::{Auth, Client as BitcoinClient, RpcApi as _};
use enclavia::Pcrs;
use serde::Deserialize;
use serde_json::json;
use tinylayer_client::{
    CoinMetadata, CoinStatus, NetworkId, RegisterRequest, RemoteEnclave, SignRequest, SignResponse,
    SignedRecovery, validate_reaction_window, verify_funding_utxo, verify_recovery, verify_status,
};
    ...
    pub async fn connect(config: &ChainConfig, network: NetworkId) -> Result<Self> {
        ...
        match config {
            ...
            ChainConfig::CoreRpc {
                ...
                Ok(Self::Core(Core::connect(rpc_url, cookie_file, network)?))
            }
        }
    }
...
impl Core {
    pub fn connect(rpc_url: &str, cookie_file: &Path, network: NetworkId) -> Result<Self> {
        validate_core_config(rpc_url, cookie_file)?;
        let client = BitcoinClient::new(rpc_url, Auth::CookieFile(cookie_file.to_owned()))
...
pub fn verify_public_history(
    ...
    require_reaction_margin(
        tip_height,
        history.last().expect("history checked non-empty").locktime,
...
pub fn require_reaction_margin(
    ...
    let minimum = tip_height
        .checked_add(minimum_reaction_blocks)
        .context("reaction margin height overflow")?;
    ensure!(
        locktime > minimum,
        "latest recovery locktime {locktime} must be greater than tip {tip_height} plus reaction margin {minimum_reaction_blocks}"
    );
    Ok(())
}
